...
def test_self_review_evidence(
    tmp_path: Path, backend: str, outcome: str, wip: bool,
) -> None:
    ...
    baseline = log_dir / "wip.diff"
    baseline.write_text("original author draft")
    ...
    def retry(output: Path, label: str, command: list[str], **kw: object) -> bool:
        prompt = str(kw["stdin"])
        evidence = Path(prompt.splitlines()[0])
        ...
        if backend in {"gemini", "agy"}:
            ...
            if wip:
                readable_baseline = evidence.parent / "wip-original.diff"
                assert readable_baseline.read_text() == "original author draft"
                assert f"`{readable_baseline}`" in prompt
                assert str(baseline) not in prompt
        else:
            assert command == backend_command(backend)
            assert evidence == log_dir / "diff-1-1.diff"
            if wip:
                assert f"`{baseline}`" in prompt
    ...
    def run() -> str:
        return self_review_subloop(
            ...
            backend=backend,
            scope_note=f"Original draft: `{baseline}`" if wip else "",
            scope_diff_file=baseline if wip else None,
        )
    ...
    assert baseline.read_text() == "original author draft"
